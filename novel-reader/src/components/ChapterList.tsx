...
import { BookmarkIcon, RefreshIcon } from './FlatIcons';
    ...
    >
      ...
      <div style={{ padding: '8px 16px', borderBottom: '1px solid var(--border-glass)' }}>
        {searchTab === '章节' ? (
          ...
          <div style={{ marginTop: 6 }}>
            ...
            <div style={{ overflow: 'hidden', transition: 'max-height 0.25s ease, opacity 0.2s ease', maxHeight: rangeOpen ? '120px' : 0, opacity: rangeOpen ? 1 : 0 }}>
                <div style={{ paddingLeft: 22, display: 'flex', alignItems: 'center', gap: 3, marginTop: 5 }}>
                  <span style={{ fontSize: '.82rem', color: 'var(--text-dim)' }}>第</span>
                  <input type="text" inputMode="numeric" minLength={1}
                    value={rangeStart ?? ''}
                    placeholder="不"
                    onClick={(e) => e.stopPropagation()}
                    onChange={(e) => {
                    ...
                    style={{
                      width: 40, padding: '3px 0', fontSize: '.82rem', textAlign: 'center',
                      background: 'var(--glass-bg)', color: 'var(--text)',
                      border: '1px solid var(--border-glass)', borderRadius: 'var(--radius-sm)',
                      outline: 'none', boxSizing: 'border-box',
                    }}
                  ...
                  <button className="btn" style={{ padding: '2px 6px', fontSize: '.78rem', lineHeight: '20px', whiteSpace: 'nowrap' }}
                  ...
                  <span onClick={(e) => { e.stopPropagation(); setRangeStart(null); setRangeEnd(null); }}
                    style={{ fontSize: '.82rem', color: 'var(--text-dim)', cursor: 'pointer', padding: '2px 6px', display: 'flex', alignItems: 'center' }}
      ...
      {searchTab === '章节' && (
        <div style={{ borderBottom: '1px solid var(--border-glass)' }}>
          <div onClick={() => setBookmarkOpen(!bookmarkOpen)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', userSelect: 'none', padding: '8px 20px' }}>
            <span style={{ fontSize: '.9rem', color: 'var(--accent)', lineHeight: 1, display: 'flex' }}>
              <svg width="12" height="12" viewBox="0 0 10 10" fill="none" style={{
                transform: bookmarkOpen ? 'rotate(90deg)' : 'rotate(0deg)',
                transition: 'transform 0.2s ease',
              }}>
                <path d="M3 1l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            ...
            {!bookmarkOpen && bookmarks.length > 0 && (
              <span style={{ fontSize: '.78rem', color: 'var(--text-dim)', marginLeft: 2 }}>{bookmarks.length}个</span>
            )}
          </div>
          <div style={{
            overflow: 'hidden',
            transition: 'max-height 0.25s ease, opacity 0.2s ease',
            maxHeight: bookmarkOpen ? `${Math.max(bookmarks.length, 1) * 36 + 16}px` : 0,
            opacity: bookmarkOpen ? 1 : 0,
          }}>
            <div style={{ padding: '0 20px 8px' }}>
              {bookmarks.length === 0 ? (
                <div style={{ fontSize: '.78rem', color: 'var(--text-dim)', padding: '4px 0' }}>暂无书签</div>
              ) : (
                bookmarks.map((bm) => (
