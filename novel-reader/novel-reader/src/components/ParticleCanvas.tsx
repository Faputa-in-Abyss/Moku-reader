...
function createParticle(W: number, H: number): Particle {
  return {
    ...
    bo: 0.12 + Math.random() * 0.35,
    to: 0.12 + Math.random() * 0.35,
    co: 0.12 + Math.random() * 0.35,
    wf: 0.005 + Math.random() * 0.015,
    wa: 0.3 + Math.random() * 0.8,
    wo: Math.random() * 6.28,
    cm: Math.random(),
...
export default function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      _mouse.x = e.clientX;
      _mouse.y = e.clientY;
      _mouse.vx = _mouse.x - _mouse.px;
      _mouse.vy = _mouse.y - _mouse.py;
    };
    const onLeave = () => {
      _mouse.x = -9999;
      _mouse.y = -9999;
    ...
    function animate() {
      ...
      for (const p of _particles) {
        ...
        if (dist < md && dist > 0) {
          ...
          const fi = Math.min(
            1,
            Math.sqrt(_mouse.vx * _mouse.vx + _mouse.vy * _mouse.vy) * 0.05
          );
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.s * p.z, 0, 6.28);
        ctx.fillStyle = grad;
        ctx.fill();

        ctx.beginPath();
        ctx.arc(p.x, p.y, p.s * p.z * 0.35, 0, 6.28);
